#!/usr/bin/env python3
"""
Cowork Session Manager (Windows)
---------------------------------
...
Usage:
    python cowork_session_cleaner_win.py                    # default: manage all sessions
    python cowork_session_cleaner_win.py --dry-run          # preview without changes
    python cowork_session_cleaner_win.py --sort size        # sort by size (default: date)
    python cowork_session_cleaner_win.py --sort name        # sort alphabetically
    python cowork_session_cleaner_win.py --archived         # show only archived sessions
    python cowork_session_cleaner_win.py --active           # show only active sessions
...
Session path on Windows:
    ~\\AppData\\Local\\Packages\\Claude_{uuid}\\LocalCache\\Roaming\\Claude\\local-agent-mode-sessions\\{outer_uuid}\\{project_uuid}\\local_{session_uuid}
"""
...
def find_sessions_root():
    """
    Locate the sessions root directory by searching for the Claude app package
    under %LOCALAPPDATA%\\Packages\\Claude_*.
    ...
    # Gather all Claude_* package directories, prefer the most recently modified
    claude_dirs = sorted(
...
def find_session_json(session_dir):
    ...
    On Windows the session directory name has a 'local_' prefix (e.g. local_{uuid}).
    The JSON file (local_{uuid}.json) is typically stored in the parent (project) directory.

    Search order:
      1. <parent>/<local_uuid>.json  (most common location on Windows)
      2. <session_dir>/<local_uuid>.json
      3. <parent>/<uuid>.json        (without local_ prefix, fallback)
      4. <session_dir>/<uuid>.json
      5. Broad scan of parent and session dir for any matching .json
    """
    session_name = session_dir.name                    # "local_{uuid}"
    session_uuid = session_name.replace("local_", "")  # "{uuid}"

    # 1. <parent>/local_{uuid}.json  (Windows standard location)
    candidate = session_dir.parent / f"{session_name}.json"
    if candidate.exists():
        return candidate

    # 2. <session_dir>/local_{uuid}.json
    candidate = session_dir / f"{session_name}.json"
    if candidate.exists():
        return candidate

    # 3. <parent>/{uuid}.json  (without prefix, macOS-style fallback)
    ...
    # 5. Broad scan: any .json whose stem matches either form
    for search_dir in (session_dir.parent, session_dir):
        if not search_dir.exists():
            continue
        for f in search_dir.iterdir():
            if f.suffix == ".json" and f.stem in (session_name, session_uuid):
                return f
...
def discover_sessions():
    ...
    Windows structure (3 levels, same depth as macOS):
        <sessions_root>\\<outer_uuid>\\<project_uuid>\\local_{session_uuid}

    The session directory is identified by the 'local_' prefix, identical to
    the macOS convention. The outer and project levels use UUID directory names
    ...
    if SESSIONS_ROOT is None or not SESSIONS_ROOT.exists():
        ...
        print(r"  %LOCALAPPDATA%\Packages\Claude_<uuid>\LocalCache\Roaming\Claude\local-agent-mode-sessions")
        sys.exit(1)

    for outer_dir in SESSIONS_ROOT.iterdir():
        if not outer_dir.is_dir():
            continue

        for project_dir in outer_dir.iterdir():
            ...
            for session_dir in project_dir.iterdir():
                ...
                # Session directories are identified by the 'local_' prefix
...
def display_sessions(sessions):
    ...
    print(f"\n{'='*90}")
    print(f"  Cowork Session Manager  [Windows]")
    print(f"  Root: {SESSIONS_ROOT}")
    ...
    if has_titles:
        print(f"  {'#':>3}  {'Status':<10} {'Last Modified':<18} {'Size':>10}  {'Title / Session ID'}")
        print(f"  {'---':>3}  {'-'*8:<10} {'-'*16:<18} {'-'*10:>10}  {'-'*40}")
    else:
        print(f"  {'#':>3}  {'Status':<10} {'Last Modified':<18} {'Size':>10}  {'Session ID'}")
        print(f"  {'---':>3}  {'-'*8:<10} {'-'*16:<18} {'-'*10:>10}  {'-'*40}")
...
def action_delete(sessions, selected, dry_run):
    ...
    for s in selected:
        try:
            ...
            # Also remove the JSON metadata if it lives outside the session dir
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog=r"""
examples:
  python cowork_session_cleaner_win.py                  # manage all sessions
  python cowork_session_cleaner_win.py --archived       # show only archived
  python cowork_session_cleaner_win.py --active         # show only active
  python cowork_session_cleaner_win.py --sort size      # sort biggest first
  python cowork_session_cleaner_win.py --dry-run        # preview, no changes
        """,
    )
    parser.add_argument(
    ...
    filter_group.add_argument(
        ...
        help="Show only active (non-archived) sessions",
    )
